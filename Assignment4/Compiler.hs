...
compileStm (SInit ty i e) = do
    ...
    v <- getVarName i
    return (s_exp ++ [s_local_set v])


compileStm (SReturn e) = do
    s_exp <- compileExp Nested e
    return (s_exp ++ [s_return])
...
compileStm (SWhile cond s) = do
    ...
    return ([s_block $ [s_loop $ s_cond ++ [s_i32_eqz] ++ [s_br_if 1] ++ s' ++ [s_br 0]]])
...
compileExp n (EIncr id@(EId i)) = do
    t <- getType id
    v <- getVarName i
    if t == Type_double then
        return $ [s_local_get v, s_f64_const 1, s_f64_add, s_local_tee v]
    else
        return $ [s_local_get v, s_i32_const 1, s_i32_add, s_local_tee v]
...
compileExp n (EAss (EId i) e) = do
    s_exp <- compileExp Nested e
    v <- getVarName i
    return $ if n == Nested then s_exp ++ [s_local_tee v] else s_exp ++ [s_local_set v]
